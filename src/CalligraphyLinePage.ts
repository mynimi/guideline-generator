...
export interface CalligraphyLinePageBasicOptions extends GridPageBasicOtions {
  lineColor?: string;
  xHeight?:number;
...
export type CalligraphyLinePageConfig = CalligraphyLinePageBasicOptions & CalligraphyLinePageExtendedOptions & CalligraphyLinePageTechOptions;
...
export class CalligraphyLinePage extends GridPage {
  ...
  private drawLineSection(parentEl:SVGElement, section:'ascender'|'base'|'descender',gridPosLine:number, lineStart:number, lineEnd:number, dividerDrawingDirection:'down'|'up'){
     ...
     */
    if(section === 'base'){    
      this.drawSolidLine(group, 'horizontal', gridPosLine, lineStart, lineEnd, color, this.#config.gridBaseLineStrokeWidth);
      if(this.#config.showXHeightIndicator){
        this.drawSolidLine(group, 'vertical', lineStart+this.#config.xHeightIndicatorStrokeWidth * 0.5, gridPosXHeightLine, gridPosLine, color, this.#config.xHeightIndicatorStrokeWidth);
      }  
    }
  ...
  private drawSlantLines(parentEl:SVGElement, gridPos:number, lineStart:number){
    ...
    for (let i = 0; i < reps; i++){
      this.drawSlantLine(parentEl, this.lineHeight, this.#config.slantAngle, startPosX, gridPos, this.#config.lineColor, this.#config.gridStrokeWidth);
