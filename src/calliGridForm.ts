type InputTypes = 'number' | 'color' | 'checkbox' | 'radio' | 'text'; // Added 'text'
...
function createColorPicker(configData: FieldConfig, id: string): HTMLDivElement {
  ...
  hexColorInput.id = `${id}_hex`; // Unique ID for hex input
  hexColorInput.setAttribute('placeholder', 'Enter hex value');
  hexColorInput.setAttribute('data-cat', configData.cat);
  hexColorInput.value = configData.initValue as string;

  const colorInput = document.createElement('input');
  colorInput.type = 'color';
  colorInput.id = `${id}_color`; // Unique ID for color input
  colorInput.setAttribute('data-cat', configData.cat);
  colorInput.value = configData.initValue as string;
  ...
  colorPickerDiv.appendChild(hexColorInput);
  ...
  Object.entries(config).forEach(([legend, fields]) => {
    ...
    Object.entries(fields).forEach(([id, configData]) => {
      ...
      if (configData.inputType === 'color') {
        input = createColorPicker(configData, id);
      } else {
        ...
        if (configData.inputType === 'checkbox' || configData.inputType === 'radio') {
          (input as HTMLInputElement).checked = configData.initValue as boolean;
        } else {
